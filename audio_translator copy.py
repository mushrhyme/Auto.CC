...
class AudioTranslator:
    ...
    async def translate_chunk(self, chunk):
        try:
            lang = detect(chunk)
            targets = self.detect_target_languages(lang)
    ...
    def detect_target_languages(self, lang):
        return {
    ...
    async def handle_audio_frames(self, frames, channels, speech_id):
        ...
        self.log_timing_on_end_of_speech()
        self.logger.info("✅ 오디오 파일 저장 완료")
        ...
        if translations:
            ...
            self.process_translation_result(
                translations, transcription, "", "", speech_id
            )
