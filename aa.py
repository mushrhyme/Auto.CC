...
class AudioTranslator:
    def __init__(self):
        ...
        if self.translation_mode == "aws":
            ...
            self.aws_language_code = "ko-KR" # en-US  ko-KR

            # (B) AWS Translate API용 소스/타f겟 코드
            #    ex) "en", "ko" 등
            self.aws_source_lang_code = "ko"
            self.aws_target_lang_code = "en"
                ...
                while not self.aws_stream_stop.is_set():
                    ...
                    if False:
                        ...
                        resp = await loop.run_in_executor(
                            None,
                            lambda: self.aws_translate.translate_text(
                                Text=text,
                                SourceLanguageCode=self.aws_source_lang_code,
                                TargetLanguageCode=self.aws_target_lang_code
                            )
